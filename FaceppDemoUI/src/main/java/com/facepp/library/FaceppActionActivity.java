...
public class FaceppActionActivity extends Activity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		if (ID == R.id.title_layout_returnRel) {
		...
		} else if (ID == R.id.landmark_edititem_3) {// 單臉跟蹤
			...
			ConUtil.isGoneKeyBoard(FaceppActionActivity.this);
		} else if (ID == R.id.landmark_imageitem_0) {// 錄像??
			isStartRecorder = !isStartRecorder;
			onclickImageItem(0, isStartRecorder);
		} else if (ID == R.id.landmark_imageitem_1) {// 3D模型
			is3DPose = !is3DPose;
			onclickImageItem(1, is3DPose);
		} else if (ID == R.id.landmark_imageitem_2) {// 調試信息
			isDebug = !isDebug;
			onclickImageItem(2, isDebug);
		} else if (ID == R.id.landmark_imageitem_3) {// 只在矩形區域做臉部偵測
			isROIDetect = !isROIDetect;
			onclickImageItem(3, isROIDetect);
		} else if (ID == R.id.landmark_imageitem_4) {// 關鍵點個數
			is106Points = !is106Points;
			onclickImageItem(4, is106Points);
		} else if (ID == R.id.landmark_imageitem_5) {//前置/後置攝像頭
			isBackCamera = !isBackCamera;
			onclickImageItem(5, isBackCamera);
			getCameraSizeList();
		} else if (ID == R.id.landmark_imageitem_6) {// 人臉屬性
		...
		} else if (ID == R.id.landmark_enterBtn) { // 最小人臉?
